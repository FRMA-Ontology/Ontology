...
if len(sys.argv) < 5 or len(sys.argv) > 6:
    print(
        ...
        "   ./face_recognition.py shape_predictor_5_face_landmarks.dat dlib_face_recognition_resnet_model_v1.dat ../examples/faces\ dlib_output_vectors.txt then add another argument if you want GUIn"
...
# os.path.join(faces_folder_path, "*.jpg")
for f in glob.glob(faces_folder_path + "/**/*.jpg"):
    print("Processing file: {}".format(f))
    image_name = str(f).split("/")[-1] # gets the filename without the path to get there
    ...
    for k, d in enumerate(dets):
        ...
        descriptor_string = [float(x) for x in str(face_descriptor).split("\n")]
        output_file.write(image_name + " - " + str(descriptor_string) + "\n")
